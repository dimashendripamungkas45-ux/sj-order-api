...
class BookingController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            \Log::info('✅ Booking created (schedule validated)', [
    ...
    public function getAvailableVehicles(Request $request)
    {
        ...
        try {
            $vehicles = Vehicle::where('is_active', true)
                              ->select('id', 'name', 'type', 'capacity', 'description', 'license_plate')
                              ->get();
    ...
    public function index(Request $request)
    {
        ...
        try {
            ...
            elseif ($user->role === 'admin' || $user->role === 'divum_admin' || $user->role === 'admin_ga') {
     ...
     *
     * 🔧 FIXED: Proper logging + formatted response
     */
    public function approveDivum(Request $request, $id)
    {
        $user = $this->getAuthUser();

        if (!$user || ($user->role !== 'admin' && $user->role !== 'divum_admin' && $user->role !== 'admin_ga')) {
            \Log::warning('⚠️ Unauthorized final approval attempt');
            return response()->json([
                'success' => false,
        ...
        try {
            \Log::info('📋 [approveDivum] START - Admin: ' . $user->name);
